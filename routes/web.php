...
// every page requires the user to be logged in
// it will redirect to the login page or the route it is supposed to go
...
Route::get('/', 'ContentsController@home')->name('home');
Route::get('/clients', 'ClientController@index')->name('clients');
//middleware('auth') will redirect to login page if someone isn't logged in
//Route::get('/clients', 'ClientController@index')->name('clients')->midleware('auth');
Route::get('/clients/new', 'ClientController@newClient')->name('new_client');
Route::post('/clients/new', 'ClientController@newClient')->name('create_client');
Route::get('/clients/{client_id}', 'ClientController@show')->name('show_client');
Route::post('/clients/{client_id}', 'ClientController@modify')->name('update_client');
...
Route::get('/about', function () {
    $response_arr = [];
    $response_arr['author'] = 'SG';
    $response_arr['version'] = '7.1';
    return $response_arr;
    //return '<h3>About</h3>';
});
